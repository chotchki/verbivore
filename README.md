...
- Vision detector (CenterNet-style, from scratch), now FUSED: the input is the screenshot plus 3 rasterized affordance planes (pointer/keyboard/scroll — evidence from live event listeners and declarative DOM, never the a11y tree, which is the label source; heat = evidence weight / specificity, so a delegation root or global key handler renders as a faint glow while an element-sized listener lands at full strength). Cross-app mAP 0.230 mean over 9 leave-one-app-out folds on a 17-app corpus, spread 0.080–0.445 — 2.6x the pixels-only 0.087 baseline, every fold's gain clearing its measured seed band. Three eval conditions per fold-seed: full prior 0.230 / degraded 0.158 / zeroed 0.075, degraded strictly between on all 33 evals — the per-channel prior dropout in training buys graceful reliance, not a cliff. The anti-shortcut gate (zeroed-prior fused >= pixels-only, the canvas condition must not regress) holds within seed bands on 8 of 9 folds; the sole miss is wordpress (-0.140), exactly the fold where pixels alone were strongest. Link AP 0.248 mean vs <=0.09 in every pixels-only rotation — the cursor bit pixels can't carry, carried in-band. The prior costs one CDP call + a rasterize at runtime (22ms measured live, one code path shared by harvest and inference). The earlier lever still stands underneath: label HONESTY beat corpus diversity beat everything else, and pointer-only links (31% of corpus links, 95% on wordpress) stay demoted to ignore-masks pending the relabel verdict.

The corpus is auto-labeled — bounding boxes and roles from the accessibility tree, effect labels from mutation/network signals with per-node ambient suppression, and links that are visually indistinguishable from their surrounding text demoted to ignore-masks instead of labels (masked uncertainty, not taught-as-background). No human ever drew a box. The two labeler bugs that corrupted training labels along the way (and the model faithfully LEARNING the corruption, twice) were the best ML lessons in the repo — the archive has the full postmortems.
