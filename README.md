...
- Vision detector (CenterNet-style, from scratch): cross-app mAP 0.087 mean over 9 leave-one-app-out folds on a 17-app corpus (spread 0.008–0.283 — conventional web transfers, dashboards and widget-demo pages don't yet; training is bit-reproducible at fixed seed, but init/shuffle seed variance swings single folds by up to 0.100 mAP, so levers are judged on seed-mean deltas, never one fold's). The measured levers, in order of what actually moved the number: label HONESTY beat corpus diversity beat everything else. Demoting pointer-only links — styled identically to their parent text, cursor:pointer their only affordance, 31% of corpus links and 95% on wordpress — took classically-styled link strata from ~0 to 0.38–0.69 and the wordpress fold's buttons to 0.652. Pixels can't learn an affordance pixels don't carry. Not in the runtime path today; its job is canvas content, where there is no tree to rank over.
