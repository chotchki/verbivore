...
- Vision detector (CenterNet-style, from scratch): cross-app mAP 0.068 mean over leave-one-app-out folds on a 17-app corpus (spread 0.022–0.336 — conventional web transfers, dashboards don't yet). The measured levers, in order of what actually moved the number: label HONESTY beat corpus diversity beat everything else. Demoting pointer-only links — styled identically to their parent text, cursor:pointer their only affordance, 31% of corpus links and 95% on wordpress — took the worst fold from 0.000 to 0.153 and link AP on classic styled links to 0.473 in the 8–12px stratum. Pixels can't learn an affordance pixels don't carry. Not in the runtime path today; its job is canvas content, where there is no tree to rank over.
