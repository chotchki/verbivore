...
- **Execution takes a context; records store variants.** Responsive breakpoints make a page a DIFFERENT UI at different widths (desktop nav vs hamburger), so what a verb was grounded under — viewport, dpr, zoom, color scheme, the same axes as the harvester's augmentation grid — is a RENDERING the verb has been grounded FOR, not the verb's identity. Tests establish an execution context once per run (base url of the deployment under test + rendering, eventually session); the executor resolves (verb, context) to the matching grounded variant, and execution/repair run at exactly that rendering. Two payoffs: records never carry a host (grounding provenance keeps its capture url, execution runs against the context's deployment), and a context with no matching variant is just another repair trigger — re-ground under that rendering and the verb grows a variant. `submit_login` stays ONE verb with desktop and mobile groundings, not two verbs.
...
- **Escape hatches, named up front:** if from-scratch training stalls, `burn-onnx` can import a pretrained backbone. If wgpu-on-Metal training iteration is unbearably slow, an RTX 2080 Ti box (5800X3D, 64GB) is available and burn's CUDA backend (CubeCL) makes that a pure backend swap — same model code, same framework, pure Rust intact. MEASURED (2026-07-22, train-bench, f32, identical seeded data): M3 Max Metal 3.06s/epoch vs Ti-in-WSL2 CUDA 11.35s/epoch — the Mac wins 3.7x, so training stays on the M3 Max and the Ti hatch is closed unless model scale forces an f16 + release-profile revisit. Burn's libtorch backend is the LAST resort behind both — it compromises the pure-Rust goal, so it's not a quiet fallback.
...
- Effect validation: catches 95% of deliberately-sabotaged actions (clicks rewired to dead pixels) with under 5% false alarms on a noisy animated page. The sabotage harness is itself a deliverable — it's the planted-violation test of this project. **MET (2026-07-23):** diff-stack heldout 1.000 catch / 0.014 FA at a TRAIN-frozen threshold (no eval peeking), 643 visible pairs over 76 urls; sabotage harness 15/15 across three fixture variants including slow-ticker pages. The road there was the real lesson: two labeler bugs (count-subtraction aliasing against periodic tickers, then ambient windows shorter than the noise period) both got LEARNED by the model before they got caught — label hygiene was the hard part, exactly as predicted below.
- Effect validation, the honest bar: the trained model must BEAT a tuned SSIM-threshold diff (burn ships SSIM as a metric, so the baseline is nearly free). If dumb structural similarity hits the numbers above, we ship the baseline and the ML moves elsewhere — boring is good. **BEATEN (2026-07-23):** on the noisy corpus ssim's score distributions overlap inverted — real badge/dot changes score BELOW its ambient noise floor — so its oracle ceiling is 0.719 catch / 0.014 FA and NO threshold passes the gates (frozen-threshold transfer: 0.754/0.095). The model earns its inference cost.
...
- End-to-end: one real app where `generate-verb "pick a date range"` produces a verb that runs deterministically and self-reports a dead click. **MET (2026-07-23):** on gitea, `crawl` proposed 9 candidates from 8 pages and `generate-verb "the search button"` grounded from the phrase alone; review-accepted verbs re-run deterministically with the trained gate judging every step. The dead-click self-report fires in the accept flow itself — the fixture's inert button proposes like any live one and review rejects it as `EffectSilence{step:0}` (proven in the `full_loop` e2e).
...
- ~~Effect model shape: siamese embedding distance vs a change-region head? Needs a spike.~~ RESOLVED (3.5/3.6): diff-stack — [before; after; |diff|] as 9 channels through one encoder. Siamese's global embeddings wash out small local changes; letting convs see the diff directly is what catches a 10px status dot through ambient animation.
